...
export const LikeButton = ({
  ...
  const isLiked = likes?.find((id) => id === userContext?.user?.id);
...
export const Details = ({ postId }: { postId?: number }) => {
  return (
    <Link to={`/p/${postId}`}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
...
export const Bookmark = ({ postId }: { postId: number }) => {
  const userContext = useUserContenxt();
  ...
  return isLoading ? (
    ...
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className={`${
        userContext?.user?.saved?.find((id) => id === postId) ? "active" : ""
      }`}
      viewBox="0 0 24 24"
      strokeWidth="1.5"
      stroke="currentColor"
